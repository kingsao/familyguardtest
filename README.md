...
本项目已配置GitHub Actions工作流，支持自动化构建包含FamilyGuard组件的OpenWrt固件。

### 工作流特点

- **定时构建**：每天22:00自动执行构建
- **代码触发**：当FamilyGuard组件代码更新时自动构建
- **手动触发**：支持通过GitHub界面手动触发构建
- **产物管理**：自动上传固件、软件包和构建信息

### 工作流文件

专用的FamilyGuard自动化构建工作流配置文件位于：
```
...
2. 启用GitHub Actions功能
3. 根据需要修改工作流配置或FamilyGuard组件代码
4. 等待自动构建或手动触发构建
5. 在GitHub Actions页面下载构建产物

### FamilyGuard组件说明

FamilyGuard是一个家庭网络管理和家长控制解决方案，包含以下组件：

- **familyguard-utils**：命令行工具集
- **familyguard-core**：核心引擎
- **luci-app-familyguard**：LuCI网页界面

有关FamilyGuard组件的详细信息，请参阅：
```
package/familyguard/README.md
```
